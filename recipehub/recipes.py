...
def get_recipes():
    """Fetch and display all recipes, categories,
    and user's bookmarked recipes."""
    ...
    for recipe in recipes:
        recipe['category_name'] = next(
            (cat['name'] for cat in categories
             if cat['_id'] == recipe['category_id']),
            "Unknown"
        )
        recipe['comment_count'] = mongo.db.comments.count_documents(
    ...
    return render_template(
        'recipes.html', recipes=recipes, categories=category_list,
        registered_users=registered_users, form=bookmark_form
    )
    ...
    if form.validate_on_submit():
        ...
        if image_file and image_file.filename != '':
            ...
            upload_folder = os.path.join(
                current_app.root_path, 'static', 'uploads', 'recipes'
            )
        ...
        logger.info(
            f"New recipe added by user {current_user.username}: "
            f"{recipe['title']}"
        )
...
def edit_recipe(recipe_id):
    ...
    if (str(recipe['created_by']) != current_user.get_id()
    ...
    if form.validate_on_submit():
        ...
        if form.remove_image.data:
            ...
            image_file.save(image_path)
            update_data["image_path"] = os.path.join(
        ...
        logger.info(
            f"Recipe updated by user {current_user.username}: "
            f"{update_data['title']}"
        )
...
def delete_recipe(recipe_id):
    """Allow the creator or an admin to delete a recipe
    and its associated comments."""
    recipe = mongo.db.recipes.find_one({"_id": ObjectId(recipe_id)})
    if recipe and (
        recipe['created_by'] == ObjectId(current_user.get_id())
        or session.get('is_admin')
    ):
        if recipe["image_path"] != DEFAULT_IMAGE_PATH:
            try:
                os.remove(os.path.join(
                    current_app.root_path, 'static', recipe["image_path"]
                ))
            except Exception as e:
                logger.error(
                    f"Error removing image {recipe['image_path']} for recipe "
                    f"{recipe_id}: {e}"
                )
        ...
        comments = list(
            mongo.db.comments.find({"recipe_id": ObjectId(recipe_id)})
        )
        ...
        logger.info(
            f"Recipe and associated comments deleted by user "
            f"{current_user.username}: {recipe['title']}"
        )
        flash(
            'Recipe and associated comments deleted! Points have been updated',
            'success'
        )
    else:
...
def view_recipe(recipe_id):
    """View a recipe's details, including comments,
    and allow users to add comments."""
    recipe = mongo.db.recipes.find_one({"_id": ObjectId(recipe_id)})
    if not recipe:
    ...
    if current_user.is_authenticated:
        ...
        if (user and 'bookmarked_recipes' in user
                and ObjectId(recipe_id) in user['bookmarked_recipes']):
            is_bookmarked = True

    bookmark_form = BookmarkForm()

    return render_template(
        'view_recipe.html', recipe=recipe, comments=comments,
        form=form, bookmark_form=bookmark_form, is_bookmarked=is_bookmarked
    )
...
def view_bookmarks():
    ...
    bookmarked_recipes = list(mongo.db.recipes.find(
        {"_id": {"$in": user.get('bookmarked_recipes', [])}}
    ))
    ...
    return render_template(
        'bookmarks.html', recipes=bookmarked_recipes, form=form
    )
...
def delete_comment(comment_id):
    ...
    if (str(comment['user_id']) != current_user.get_id()
        ...
        return redirect(
            url_for('recipes.view_recipe', recipe_id=comment['recipe_id'])
        )
