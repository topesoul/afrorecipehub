...
def register():
    ...
    if form.validate_on_submit():
        username = form.username.data.strip()  # Strip spaces
        existing_user = mongo.db.users.find_one({"$or": [{"email": form.email.data}, {"username": username}]})
        if existing_user:
        ...
        user = {
            "username": username,  # Save the stripped username
...
def change_username(username):
    ...
    if form.validate_on_submit():
        new_username = form.username.data.strip().lower()  # Strip spaces and convert to lower case for comparison

        if new_username == current_user.username.lower():  # Compare case-insensitively
        ...
        if existing_user:
            ...
            mongo.db.users.update_one(
                ...
                {"$set": {"username": form.username.data.strip()}}  # Save original case version of username
            )
